...
void read_anglepattern(float* gcumf, int nn)
{
	...
	{
		case 1:
			sprintf(filename, "MCF_input/tissue_anglepattern.txt"); //sprintf(filename,"%s0.5_%dnm.txt",address,index);  // can be modified
			//printf("Read tissue_anglepattern.txt\n");
			break;
		case 2:
			sprintf(filename, "MCF_input/tissue_anglepattern2.txt"); //sprintf(filename,"%s0.5_%dnm.txt",address,index);  // can be modified
...
int read_data(SimulationStruct** simulations)
{
	...
	float medium_n = 1.0;//1.60;
	// refractive index of tissue
	float tissue_n = 1.4;//1.33;
	...
	para = (char*)malloc(30);
	para[0] = '\0';
	strcat(para,"MCF_input/DRSparameter");
